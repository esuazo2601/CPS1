...
temp_ref_celsius = 1423 # Temperatura de referencia en Celsius
...
op1_folder = './OP1'
img_op1 = [f for f in os.listdir(op1_folder) if f.endswith('.tiff')]
...
for i, imagen in enumerate(img_op1):
    # Cargar la imagen
    print(f"Procesando imagen {i}")
    ruta_imagen = os.path.join(op1_folder, imagen)
...
with open('OP1_errores_sistematicos.txt', 'w') as f:
...
with open('OP1_errores_aleatorios.txt', 'w') as f:
